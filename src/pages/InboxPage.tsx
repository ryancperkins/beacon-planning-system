...
import { Inbox as InboxIcon, AlertCircle, CheckCircle, Clock, Eye } from "lucide-react";
...
export default function InboxPage() {
  ...
  const quickAction = useMutation({
    mutationFn: async ({ id, newStatus, actionLabel }: { id: string; newStatus: string; actionLabel: string }) => {
      ...
      // If approving, create token transaction
      if (newStatus === "Approved") {
        const init = allInitiatives.find((i: any) => i.id === id);
        if (init?.token_cost_estimate) {
          await supabase.from("token_transactions").insert({
            initiative_id: id,
            ministry_id: init.ministry_id,
            amount: init.token_cost_estimate,
          ...
          return (
            ...
            >
              ...
              <span className="font-body" style={{
                ...
                background: isActive ? "var(--accent-bg-strong)" : "var(--bg-inset)",
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {activeTab === "intake" && (
                      <>
                        ...
                        <button className="font-body" style={btnAccent} onClick={() => quickAction.mutate({ id: init.id, newStatus: "Approved", actionLabel: "Status changed to Approved" })}>Approve</button>
                      </>
                    )}
                    {activeTab === "needsinfo" && (
