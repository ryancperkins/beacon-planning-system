...
import { ArrowLeft, Send, Clock, ChevronDown, Upload, Edit, CheckCircle, Eye, EyeOff } from "lucide-react";
...
import { DetailSkeleton } from "@/components/beacon/LoadingSkeleton";
...
export default function InitiativeDetailPage() {
  ...
  const { user } = useAuth();
  const { canApprove, canEditInitiative, canToggleVisibility, canAddNotes } = usePermissions();
  ...
  const { data: notes = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("initiative_notes").select("*").eq("initiative_id", id!).order("created_at", { ascending: false });
      return data || [];
    },
    enabled: !!id,
  });
  ...
  const { data: activity = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("initiative_activity").select("*").eq("initiative_id", id!).order("created_at", { ascending: false });
      return data || [];
    },
    enabled: !!id,
  });
  ...
  const addNote = useMutation({
    mutationFn: async (content: string) => {
      const { error } = await supabase.from("initiative_notes").insert({ initiative_id: id!, author_id: user!.id, content });
      if (error) throw error;
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["initiative-notes", id] }); setNoteText(""); toast.success("Note added"); },
  });
  ...
  const updateStatus = useMutation({
    mutationFn: async (newStatus: string) => {
      ...
      if (error) throw error;
      await supabase.from("initiative_activity").insert({ initiative_id: id!, actor_id: user!.id, action: `Status changed to ${newStatus}` });
    },
    onSuccess: () => {
  ...
  const approveAction = useMutation({
    mutationFn: async () => {
      ...
      if (error) throw error;
      await supabase.from("initiative_activity").insert({ initiative_id: id!, actor_id: user!.id, action: "Status changed to Approved" });
      if (initiative?.token_cost_estimate) {
        await supabase.from("token_transactions").insert({ initiative_id: id!, ministry_id: initiative.ministry_id, amount: initiative.token_cost_estimate, reason: "Initiative approved" });
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["initiative", id] });
  ...
  const toggleVisibility = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("initiatives").update({ visible_to_all_staff: !initiative!.visible_to_all_staff }).eq("id", id!);
      if (error) throw error;
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["initiative", id] }); toast.success("Visibility updated"); },
  });
  ...
  const toggleChecklistItem = useMutation({
    mutationFn: async ({ index, checked }: { index: number; checked: boolean }) => {
      ...
      if (error) throw error;
      await supabase.from("initiative_activity").insert({ initiative_id: id!, actor_id: user!.id, action: `Marked '${list[index].label}' as ${checked ? "complete" : "incomplete"}` });
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["initiative", id] }); queryClient.invalidateQueries({ queryKey: ["initiative-activity", id] }); },
  });
  ...
  const showApprove = canApprove && ["Reviewed", "Intake"].includes(initiative.status);
  const canEdit = canEditInitiative(initiative.ministry_id);
  const canToggleVis = canToggleVisibility(initiative.ministry_id);
  const canNote = canAddNotes(initiative.ministry_id);
    ...
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="font-display" style={{ fontSize: 22, fontWeight: 700, color: "var(--text-primary)", letterSpacing: "-0.03em" }}>{initiative.title}</h1>
            <div className="flex flex-wrap items-center gap-2 mt-1">
              ...
              {initiative.visible_to_all_staff && (
                <span className="font-body flex items-center gap-1 px-2 py-0.5 rounded-full" style={{ fontSize: 10, fontWeight: 600, background: "var(--accent-bg)", color: "var(--accent-text)", border: "1px solid var(--accent-border)" }}>
                  <Eye size={10} /> Shared
                </span>
              )}
            </div>
        ...
        <div className="flex items-center gap-2 self-start">
          {/* Visibility toggle */}
          {canToggleVis && (
            <button onClick={() => toggleVisibility.mutate()} className="flex items-center gap-1 px-3 py-1.5 rounded-lg font-body" style={{ border: "1px solid var(--border)", background: "var(--bg-surface)", color: "var(--text-tertiary)", fontSize: 11.5, cursor: "pointer" }}>
              {initiative.visible_to_all_staff ? <EyeOff size={12} /> : <Eye size={12} />}
              {initiative.visible_to_all_staff ? "Hide from staff" : "Share with staff"}
            </button>
          )}
            ...
            <div className="relative">
              ...
              {showStatusMenu && (
                <div className="absolute right-0 top-full mt-1 z-50 rounded-xl overflow-hidden" style={{ background: "var(--bg-elevated)", border: "1px solid var(--border)", boxShadow: "var(--shadow-dropdown)", minWidth: 160, padding: 4 }}>
                  {STATUS_FLOW.map((s) => (
                    <button key={s} onClick={() => updateStatus.mutate(s)} className="w-full text-left px-3 py-1.5 rounded-lg font-body block" style={{ border: "none", background: initiative.status === s ? "var(--accent-bg)" : "transparent", color: initiative.status === s ? "var(--accent-text)" : "var(--text-secondary)", fontSize: 12, cursor: "pointer" }}>{s}</button>
                  ))}
                </div>
              )}
            </div>
          )}
          {!canEdit && <StatusChip status={initiative.status} />}
      ...
      <div className="flex flex-col lg:flex-row gap-5">
        ...
        <div className="flex-1 flex flex-col gap-5 min-w-0">
          <GlassPanel style={{ padding: 20 }}>
            ...
            <p className="font-body" style={{ fontSize: 13, color: initiative.initiative_brief ? "var(--text-secondary)" : "var(--text-muted)", lineHeight: 1.6 }}>{initiative.initiative_brief || "No brief generated yet."}</p>
          </GlassPanel>
              ...
              <div className="flex flex-col gap-2">
                {checklistItems.map((item: any, idx: number) => (
                  <label key={idx} className="flex items-center gap-2 font-body cursor-pointer" style={{ fontSize: 12.5, color: item.checked ? "var(--text-muted)" : "var(--text-secondary)", textDecoration: item.checked ? "line-through" : "none" }}>
                    <input type="checkbox" checked={item.checked || false} onChange={(e) => canEdit && toggleChecklistItem.mutate({ index: idx, checked: e.target.checked })} disabled={!canEdit} style={{ accentColor: "var(--accent)" }} />
                    {item.label}
                  </label>
                ))}
              </div>
            ...
            {!strategy ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {(strategy.tiers || []).map((tier: any, idx: number) => (
                  <div key={tier.name} className="rounded-xl p-4" style={{ border: tier.selected ? "1px solid var(--accent-border)" : "1px solid var(--border)", background: tier.selected ? "var(--accent-bg)" : "var(--bg-inset)", boxShadow: tier.selected ? "var(--accent-glow)" : "none" }}>
            ...
            <div className="flex flex-wrap gap-1.5">
              {initiative.channels_requested && initiative.channels_requested.length > 0 ? (
                initiative.channels_requested.map((ch: string) => (
                  <span key={ch} className="font-body px-2.5 py-1 rounded-full" style={{ fontSize: 11, background: "var(--bg-inset)", color: "var(--text-tertiary)", border: "1px solid var(--border)" }}>{ch}</span>
                ))
              ) : (
              ...
              <div className="flex flex-col">
                {activity.map((a: any, idx: number) => (
                  <div key={a.id} className="py-2" style={{ borderBottom: idx < activity.length - 1 ? "1px solid var(--border)" : "none" }}>
                    ...
                    <span className="font-body" style={{ fontSize: 10.5, color: "var(--text-muted)" }}>{format(new Date(a.created_at), "MMM d, yyyy 'at' h:mm a")}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="flex flex-col gap-2 mb-3">
              {notes.length === 0 ? (
                ...
                <div key={note.id} className="rounded-lg p-3" style={{ background: "var(--bg-inset)", border: "1px solid var(--border)", borderRadius: 10 }}>
                  ...
                  <span className="font-body mt-1 block" style={{ fontSize: 10.5, color: "var(--text-muted)" }}>{format(new Date(note.created_at), "MMM d 'at' h:mm a")}</span>
                </div>
              ))}
            </div>
            {canNote && (
              <div className="flex gap-2">
                <input value={noteText} onChange={(e) => setNoteText(e.target.value)} placeholder="Add a note..." className="font-body flex-1 bg-transparent outline-none" style={{ padding: "8px 10px", borderRadius: 10, border: "1px solid var(--border)", background: "var(--bg-input)", color: "var(--text-primary)", fontSize: 12 }} onKeyDown={(e) => { if (e.key === "Enter" && noteText.trim()) addNote.mutate(noteText.trim()); }} />
                <button disabled={!noteText.trim()} onClick={() => noteText.trim() && addNote.mutate(noteText.trim())} className="flex items-center justify-center rounded-lg accent-gradient accent-glow" style={{ width: 34, height: 34, border: "none", color: "var(--accent-on)", cursor: noteText.trim() ? "pointer" : "not-allowed", opacity: noteText.trim() ? 1 : 0.5 }}>
                  <Send size={12} />
                </button>
              </div>
            )}
